"""Run the Mistral Medium 3.5 LLM recommender across multiple seeds.
...
from src.models.llm_mistral import MistralLLMRecommender
...
def main():
    ...
    if not MISTRAL_API_KEY:
        print("ERROR: MISTRAL_API_KEY not set. Set it via environment variable.")
        return
    ...
    raw_results = []
    output_csv = paths.RESULTS_DIR / "mistral_llm_runs.csv"
    ...
    if output_csv.exists():
        try:
            ...
            print(f"Loaded {len(raw_results)} existing Mistral runs from {output_csv}")
        except Exception:
            pass

    for seed in run_seeds:
        if any(r["model"] == "llm_mistral" and r["seed"] == seed for r in raw_results):
            print(f"[Skip] llm_mistral seed {seed} already ran")
            continue

        print(f"\n===== Mistral Medium 3.5 LLM | Seed {seed} =====", flush=True)
        set_seed(seed)

        model = MistralLLMRecommender(api_key=MISTRAL_API_KEY, seed=seed)
        model.fit(splits)
        ...
        grp.to_csv(paths.RESULTS_DIR / f"llm_mistral_s{seed}_per_group.csv", index=False)
        fair.to_csv(paths.RESULTS_DIR / f"llm_mistral_s{seed}_fairness.csv", index=False)
