"""Run the Gemini LLM recommender across multiple seeds in parallel with DeepSeek.
...
from src.models.llm_gemini import GeminiLLMRecommender
...
def main():
    ...
    if not GEMINI_API_KEY:
        print("ERROR: GEMINI_API_KEY not set. Set it via environment variable.")
        return
    ...
    raw_results = []
    output_csv = paths.RESULTS_DIR / "gemini_llm_runs.csv"
    ...
    if output_csv.exists():
        try:
            ...
            print(f"Loaded {len(raw_results)} existing Gemini runs from {output_csv}")
        except Exception:
            pass

    for seed in run_seeds:
        # Skip if already ran
        if any(r["model"] == "llm_gemini" and r["seed"] == seed for r in raw_results):
            print(f"[Skip] llm_gemini seed {seed} already ran")
            continue

        print(f"\n===== Gemini LLM | Seed {seed} =====", flush=True)
        set_seed(seed)

        model = GeminiLLMRecommender(api_key=GEMINI_API_KEY, seed=seed)
        model.fit(splits)
        ...
        grp.to_csv(paths.RESULTS_DIR / f"llm_gemini_s{seed}_per_group.csv", index=False)
        fair.to_csv(paths.RESULTS_DIR / f"llm_gemini_s{seed}_fairness.csv", index=False)
    ...
    print("\nGemini LLM experiment complete.", flush=True)
